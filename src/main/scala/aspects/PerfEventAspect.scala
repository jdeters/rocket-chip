...
class PerfEventAspect (tree: Tree) extends Aspect(tree) {
  //adding events to the Data Cache
  after(init"DCacheModuleImpl", q"{EventFactory(${"DCache release"}, () => edge.done(tl_out_c), 0x14)}")

  //adding events to the Non-Blocking Cache
  after(init"NonBlockingDCacheModule(this)", q"{EventFactory(${"DCache release"}, () => edge.done(tl_out.c), 0x14)}")

  //adding event to the Instruction Cache
  after(init"ICacheModule(this)", q"{EventFactory(${"ICache blocked"}, () => !(io.resp.valid || RegNext(io.resp.valid)), 0xc)}")

  //adding events to the Core and connecting them to the CSR
  after(init"RocketImpl", q"""
